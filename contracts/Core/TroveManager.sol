...
contract TroveManager is AGLBase, CheckContract, ITroveManager {
	...
	modifier onlyBorrowerOrRedemption() {
		if(msg.sender != address(redemptor) &&  msg.sender != borrowerOperationsAddress){
			revert CallerNotBorrowerOrRedemptor();
		}
		_;
	}

	modifier onlyRedemption() {
		if(msg.sender != address (redemptor)){
			revert CallerNotRedemptor();
		}
		_;
	}
	...
	function setRedemptorAddress(address _redemptorAddress) external onlyOwner {
		redemptor = IRedemption(_redemptorAddress);
	}
	...
	function closeTroveByRedemptor(
		address _asset,
		address _borrower
	) onlyRedemption external override{
	...
	function setTroveCollAndDebt(address _borrower, address _asset, uint256 _newDebt, uint256 _newColl) onlyRedemption external override {
