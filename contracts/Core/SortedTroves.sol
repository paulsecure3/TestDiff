...
contract SortedTroves is OwnableUpgradeable, CheckContract, ISortedTroves {
	...
	function reInsert(
		...
		ITroveManager troveManagerCached = troveManager;
		_requireCallerIsBOorTroveMorTroveT(troveManagerCached);
		// List must contain the node
		require(contains(_asset, _id), "SortedTroves: List does not contain the id");
		// NICR must be non-zero
	...
	function _requireCallerIsBOorTroveMorTroveT(ITroveManager _troveManager) internal view {
		require(
			msg.sender == borrowerOperationsAddress || msg.sender == address(_troveManager) || msg.sender == address(redemptorAddress),
			"SortedTroves: Caller is neither BO nor TroveM"
		);
	}
}
