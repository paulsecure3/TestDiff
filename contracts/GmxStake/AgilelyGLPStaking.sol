...
import { IAgilelyGMXStaking } from "../Interfaces/IAgilelyGMXStaking.sol";
import { TransferHelper, IERC20 } from "../Dependencies/TransferHelper.sol";
import { FullMath } from "../Dependencies/FullMath.sol";
import { IPriceFeedV2 } from "../Interfaces/IPriceFeedV2.sol";

contract AgilelyGLPStaking is IAgilelyGMXStaking, OwnableUpgradeable {
	uint256 private constant PRECISION = 1e27;
	bool private reentrancy;

	address public agilelyTreasury;
	address public sGLP;
	...
	string public constant NAME = "AgilelyGLPStaking";

	IGMXRewardTracker public constant fGLP =
		IGMXRewardTracker(0x5294A1B34Bd717EAC8b4eF2918B07e006d66DcD7);
	...
	function setUp(
		address _agilelyTreasury,
		address _sGLP,
		address _gmxRewardRouterV2,
		address _feeGlpTrackerRewards
	)
		external
		onlyActiveAddress(_agilelyTreasury)
		...
		initializer
	{
		...
		agilelyTreasury = _agilelyTreasury;
		sGLP = _sGLP;
	...
	function retrySendTreasuryFund() external onlyOwner {
		...
		(bool success, ) = agilelyTreasury.call{ value: toTreasury }("");
		if (!success) {
			revert ETHTransferFailed(agilelyTreasury, toTreasury);
		}
	}
	...
	function _payTreasuryFee(uint256 _rawRewards) internal {
		...
		(bool success, ) = agilelyTreasury.call{ value: toTheTreasury }("");
		if (!success) {
			failedToSend += toTheTreasury;
			emit FailedToSendETH(agilelyTreasury ,toTheTreasury);
		}
	}
}
