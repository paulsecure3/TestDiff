...
import { EnumerableSetUpgradeable as Set } from "@openzeppelin/contracts-upgradeable/utils/structs/EnumerableSetUpgradeable.sol";
...
contract SavingModule is ISavingModule, OwnableUpgradeable, Shareable {
	...
	function setUp(
		...
		maxUSDA = 1_000_000e18;
		releaseDate = block.timestamp;
	}
	function increaseLockDaysTo(uint256 _lockId, uint128 _lockDays) external override{}

	function depositUSDA(uint256 _amount) public virtual onlyVRROrOwner {
		vrrGeneratedRevenueTracker += _amount;
		lastVRRAmountReceived = _amount;

		_depositUSDA(_amount);
		emit USDAReceveidFromVRR(_amount);
	}

	function _depositUSDA(uint256 _amount) internal {
		if (_amount == 0) return;
		...
		if (toSavingModule < _amount) {
			IERC20(usda).transfer(emergencyReserve, _amount - toSavingModule);
		}
	...
	function createLock(
		...
		if (_spSupply() + _amount > maxUSDA) revert PoolIsFull();
	...
	function _claim(uint256 _lockId, Lock storage _lock) internal {
		...
		uint256 lockBalance = stabilityPool.getCompoundedUSDADeposit(_lockId);
		...
		IERC20(usda).transfer(msg.sender, interest);

		if (missedInterest != 0) {
			IERC20(usda).transfer(emergencyReserve, missedInterest);
		}
	...
	function _exit(uint256 _lockId, Lock storage _lock) internal {
		...
		uint256 returningAmountLog = stabilityPool.getCompoundedUSDADeposit(
			_lockId
		);
	...
	function _spSupply() internal view returns (uint256) {
		return IERC20(usda).balanceOf(address(stabilityPool));
	}
	...
	function setMaxSupply(uint256 _maxUSDA) external onlyOwner {
		maxUSDA = _maxUSDA;
	}
	...
	function withdrawOldVRR() external onlyOwner {
		IERC20(usda).transfer(
			emergencyReserve,
			IERC20(usda).balanceOf(address(this)) - rewardAllocation
		);
	}
}
