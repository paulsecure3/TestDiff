...
import { Lock } from "../InterestManager/model/SavingModuleModel.sol";
...
interface ISavingModule {
	...
	event PoolUpdated(uint256 addedReward, uint256 totalAllocatedUSDA);
	event UserClaimed(
		address indexed user,
		uint256 indexed lockId,
		uint256 claimed
	...
	event ExitLock(
		address indexed user,
		uint256 indexed lockId,
		uint256 usdaAmountReturned
	);

	event LockAutoLockChanged(uint256 indexed lockId, bool autolock);
	event USDAReceveidFromVRR(uint256 receivedUSDA);
	 ...
	 */
	function depositUSDA(uint256 _amount) external;
